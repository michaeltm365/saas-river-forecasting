"""Canonical all-sites LSTM: calendar windows, causal filling, status availability."""
...
def main():
    ...
    ap.add_argument("--variant", choices=["availability"], default="availability")
    args = ap.parse_args()
    if args.prepare:
        ...
        audit = dict(rows=len(df), features=feats, sequences=len(y),
                     ...
                     causal_filling=True)
        (ROOT / "results/paper").mkdir(parents=True, exist_ok=True)
        (ROOT / "results/paper/preflight_data.json").write_text(json.dumps(audit, indent=2))
    ...
    runner.OUT = ROOT / "results/paper" / ("lstm_" + args.variant)
