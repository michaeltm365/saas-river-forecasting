...
def sensor_predictions(seed=42):
    folder=REPO/"results/paper/predictions"
    df=pd.read_csv(folder/f'rgcn_seed{seed}_day3.csv',parse_dates=['date'])
    ...
    order=pd.read_csv(REPO/'results/paper/predictions/stream_order.csv')[['NHDPlusID','StreamOrde']].drop_duplicates()
...
def plot_raw_map(table):
    ...
    cache=REPO/'results/paper/map_inputs'
